...
public class FragmentContractors extends Fragment implements View.OnClickListener
{
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ((MainActivity)getActivity()).getSupportActionBar().setTitle("Contractors");
    ...
    private class ContractorsUploadTask extends AsyncTask<Void, Void, Boolean>
    {
        ...
        protected Boolean doInBackground(Void... voids) {
            ...
            {
                CompletableFuture<ParseObject> REFERENCE = CompletableFuture.supplyAsync(()->uploadPrimary.contractors_upload(data, contractors_extractStringsToTags()));
                try {reference = REFERENCE.get();}
                catch (ExecutionException e) {e.printStackTrace();}
                catch (InterruptedException e) {e.printStackTrace();}

                CompletableFuture<Void> cf = CompletableFuture.supplyAsync(()->remarksUpload.contractors_remarks_upload(labels, reference))
                        .thenAccept(result->{results.add(result);})
                        .thenApplyAsync(dat->imageUpload.contractors_image_upload(reference, imageList))
                        .thenAccept(result->{results.add(result);})
                        .thenApplyAsync(dat->fileUpload.contractors_file_upload(reference, filesList))
                        .thenAccept(result->{results.add(result);});
                try {cf.get();}
                catch (ExecutionException e) {e.printStackTrace();}
                catch (InterruptedException e) {e.printStackTrace();}
